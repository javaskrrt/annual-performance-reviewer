...
async function getAuthorEmails(repoPath) {
...
async function getCommits(repoPath) {
  ...
  const rawCommits = out.split(COMMIT_SEPARATOR).map((s) => s.trim()).filter(Boolean);
...
async function listSubdirs(root) {
  const entries = await Bun.file(root).exists() ? await Array.fromAsync(new Bun.Glob("*").scan({ cwd: root, onlyFiles: false })) : [];
  return entries.map((p) => join(root, p)).filter((p) => !p.includes("node_modules") && !p.includes(".git"));
}
...
program2.name("javaskrrt").description("Generate a performance review from your git commits.").version("0.1.0");
program2.command("generate-performance-review").description("Scan repos in the current directory and generate a review.").option("-r, --root <path>", "Root directory containing repos", process.cwd()).option("--since <date>", "Only include commits since this date (git understands many formats)", "1 year ago").option("--until <date>", "Only include commits until this date (default: now)").option("--endpoint <url>", "Server endpoint to send commits to", process.env.JAVASKRRT_ENDPOINT || "http://localhost:3000/api/performance-review").option("--out <path>", "Write consolidated commit text to file (default: ./javaskrrt_commits.txt)").option("--dry-run", "Do not call server, just write file").action(async (opts) => {
  const root = opts.root;
  const endpoint = opts.endpoint;
  const since = opts.since;
  const until = opts.until;
  const outPath = opts.out || join2(root, "javaskrrt_commits.txt");
...
`);
  ...
  for (const repo of repos) {
    const emails2 = await getAuthorEmails(repo);
    emails2.forEach((e) => emailsSet.add(e));
  }
  ...
  const { selectedEmails } = await import_prompts.default({
    type: "multiselect",
    name: "selectedEmails",
...
`);
  ...
  if (!filtered.length) {
    console.error("No matching commits found for selected email(s) and date range.");
    process.exit(1);
  }
...
`);
    ...
    console.log(section("Goals for the upcoming year (and how)?", data.goals));
    console.log(`
\uD83E\uDDC3 go forth and self-promote.
`);
  } catch (err) {
    console.error(`❌ Failed to call server: ${err.message}`);
    console.log(`You still have the raw commit dump at: ${outPath}`);
    process.exit(1);
  }
});
program2.parse(process.argv);
function filterCommits(commits, emails, since, until) {
  const emailSet = new Set(emails.map((e) => e.toLowerCase()));
  const sinceDate = safeParseDate(since);
  const untilDate = until ? safeParseDate(until) : null;
  return commits.filter((c) => {
    if (!emailSet.has(c.authorEmail.toLowerCase()))
      return false;
    const d = new Date(c.dateISO).getTime();
    if (Number.isFinite(sinceDate) && d < sinceDate)
      return false;
    if (untilDate && Number.isFinite(untilDate) && d > untilDate)
      return false;
    return true;
  });
}
...
function toConsolidatedText(commits, range) {
  const header = [
    "JAVASKRRT PERFORMANCE REVIEW INPUT",
    `Date range: since=${range.since}${range.until ? `, until=${range.until}` : ""}`,
...
`);
  const body = commits.map((c) => {
    const firstLine = `- [${c.repoName}] ${c.dateISO} ${c.hash.slice(0, 7)} ${c.subject}`;
